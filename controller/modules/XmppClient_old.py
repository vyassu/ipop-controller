...
class XmppClient(ControllerModule,sleekxmpp.ClientXMPP):
    def __init__(self,CFxHandle,paramDict,ModuleName):
        ...
        tincanparams = self.CFxHandle.queryParam("Tincan")
        self.ipop_interfaces = tincanparams["vnets"]
        if (self.vpn_type == "GroupVPN"):
            ...
            for k in range(len(self.ipop_interfaces)):
                self.uid.append(self.ipop_interfaces[k]["uid"])
    ...
    def get_interface_name(self,uid):
         for k in range(len(self.ipop_interfaces)):
             if uid in self.ipop_interfaces[k]["uid_ip4_table"].keys():
                 return self.ipop_interfaces[k]["ipoptap_name"]
         return self.ipop_interfaces[0]["ipoptap_name"]                 # TO DO
    ...
    def MsgListener(self,msg):
        ...
        if (msg_type == "regular_msg"):
            ...
            try:
                ...
                if peer_uid not in self.uid:                        #if (peer_uid != self.uid):
                    ...
                    if target_uid not in self.uid:                  #if (target_uid == self.uid):
                    ...
                    msg["interface_name"] = self.get_interface_name(peer_uid)
                    if (self.vpn_type == "GroupVPN"):
                ...
                self.log("advt_payload: {0}".format(payload),severity=log_level)
                
        # compare uid's here , if target uid does not match with mine do nothing.
        # have to avoid loop messages.
        if target_uid in self.uid:              #if (target_uid == self.uid):
            ...
            if (msg_type == "con_req"):
                ...
                msg["data"] = recvd_data
                msg["type"] = "con_req"
                msg["interface_name"] = self.get_interface_name(sender_uid)
    ...
    def processCBT(self, cbt):
        ...
        if (cbt.action == "DO_SEND_MSG"):
            ...
            if (method == "con_req"):
                ...
                msg_payload = self.uid[interface_index]+"#"+data
        ...
        if (len(self.uid) !=0):
            for peer in self.xmpp_peers.keys():
                ...
                if (send_advt == True):
                    ...
                    for ele in self.uid:
                        msg_load = str(ele) + "#" + str(self.jid_uid[peer][0])
