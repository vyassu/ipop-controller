...
class ConnectionManager(ControllerModule):
    ...
    def __init__(self, CFxHandle, paramDict, ModuleName):
        ...
        for k in range(len(tincanparams["vnets"])):
            interface_name = tincanparams["vnets"][k]["ipoptap_name"]
            ...
            self.connection_details[interface_name]["xmpp_client_code"] = tincanparams["vnets"][k]["xmpp_code"]
            self.connection_details[interface_name]["uid"]       = tincanparams["vnets"][k]["uid"]
        self.CMConfig = paramDict
    ...
    def clean_connections(self, interface_name):
        ...
        for uid in successors:
            if time.time() > links["successor"][uid]["ttl"]:
                ...
                self.registerCBT('Logger', 'info', log)
                self.registerCBT("BaseTopologyManager","UpdateConnectionDetails",{"uid":uid,"interface_name":interface_name,"msg_type":"remove_peer"})
    ...
    def clean_on_demand(self,interface_name):
        for uid,link_details in self.connection_details[interface_name]["on_demand"].items():
            ...
            if link_details["stats"]["rate"] >= self.CMConfig["threshold_on_demand"]:
        ...
        if cbt.action == "clean_connection":
           self.clean_connections(cbt.data.get("interface_name"))
            ...
            self.registerCBT("BaseTopologyManager", "UpdateConnectionDetails",
                             {"uid": uid, "interface_name": interface_name, "msg_type": "add_peer"})
            ...
            for interface_name in self.connection_details.keys():
                ...
                self.registerCBT("Visualizer","link_details",msg)
        else:
    ...
    def timer_method(self):
        ...
        # Refresh IPOP Interface Table
        if self.int_counter % self.CMConfig["interval_management"] == 0:
            self.registerCBT('BaseTopologyManager', 'get_interface_details', "")
        '''
